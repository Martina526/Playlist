...
******************************************************/
   ...
   private Node<T> insert(Node<T> p, T toInsert, T toInsertvalue)
   {
      if (p == null)
         return new Node<T>(toInsert, toInsertvalue);

      if (compare(toInsert, p.data) == 0)
      	return p;

      if (compare(toInsert, p.data) < 0)
         p.left = insert(p.left, p.left.value, toInsert);
      else
         p.right = insert(p.right, p.right.value, toInsert);
 ...
 **************************************************/
   ...
   public ArrayList sortAsDecrease(int num)
   { 
  	  ArrayList aaa = new ArrayList();
  	  decreaseHelper(root, aaa, num);
  	  return aaa;
   }
   private ArrayList decreaseHelper(Node r, ArrayList aaa, int num)
   {
	   int flag = 0;
      ...
      if (r != null && flag == 0)
      {
         ...
         decreaseHelper(r.right, aaa, num);
         aaa.add(r.value);
         if(aaa.size() >= num){
        	 flag = 1;
        	 }
         decreaseHelper(r.left, aaa, num);      
      }
      return aaa;
   }
   public int findmini()
   {
      Node mini = miniHelper(root);
      return (int)mini.data;
   }
   private Node miniHelper(Node r)
   {
      if (r != null)
      {
         miniHelper(r.left);
 ...
 **************************************************/
   ...
   private Node<T> cloneHelper(Node<T> p)
   {
      if(p == null)
         ...
         return new Node<T>(p.data, p.value, cloneHelper(p.left), cloneHelper(p.right));
 ...
 **************************************************/
  ...
  //This method restores a BST given preorder and inorder traversals
   public void restore(T[] pre, T[] prev, T[] in)
   {
      root = restore(pre, prev, 0, pre.length-1, in, 0, in.length-1);
   }
   private Node<T> restore(T[] pre, T[] prev, int preL, int preR, T[] in, int inL, int inR)
   {
      if(preL <= preR)
      {
         int count = 0;
         ...
         Node<T> tmp = new Node<T>(pre[preL], prev[preL]);
         tmp.left = restore(pre, prev, preL+1, preL + count, in, inL, inL +count-1);
         tmp.right = restore(pre, prev, preL+count+1, preR, in, inL+count+1, inR);
         return tmp;
      }
      else
         return null;
...
/*
   private class Node<T>
   {
      ...
      public Node(T data, Node<T> l, Node<T> r, T v)
      {
         left = l; right = r;
         this.data = data;
         this.value = v;
      ...
      public Node(T data,T v)
      {
         this(data, null, null,v);
      }
...
*/ 
   private class Node<T>
   {
      ...
      public Node(T data, T value, Node<T> l, Node<T> r)
      {
         left = l; right = r;
         this.data = data;
         this.value = value;
